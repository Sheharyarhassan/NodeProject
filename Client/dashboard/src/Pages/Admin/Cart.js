...
const Cart = () => {
  ...
  const handleRemoveItem = async (id) => {
    ...
    try {
      if (userDetails?.id) {
        const response = await api.delete(`http://localhost:5000/api/cart/remove/${userDetails?.id}?itemId=${id}`)
        ...
        const response = await api.delete(`http://localhost:5000/api/cart/remove/${guestId}?itemId=${id}`)
        if (response.status === 200) {
          toast.success(response?.data?.message || 'Item Removed', {
            onClose: () => {
              setLoading(false);
