...
function Index() {
  ...
  useEffect(() => {
    api.get(`${process.env.REACT_APP_BASE_URL}book/getFilters`).then(response => {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const response = await api.post(`${process.env.REACT_APP_BASE_URL}book/GetAll?page=${currentPage}&limit=${perPage}&title=${title}&sortOrder=${sort}`, {
  ...
  const updateStatus = async (active, id) => {
    setLoading(true);
    try {
      await api.patch(`${process.env.REACT_APP_BASE_URL}book/${active ? 'Deactivate' : 'Activate'}/${id}`, {},
        {isAdmin: true})
      fetchData();
    } catch (error) {
      console.log(error);
